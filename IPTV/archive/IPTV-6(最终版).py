...
# ─────────────────────────────────────────────
#  预编译正则（避免重复编译，删除无调用的_CCTV_NORM_RE，新增CCTV匹配正则）
# ─────────────────────────────────────────────
...
# 预编译CCTV标准化正则（优化：避免函数内重复编译）
_CCTV_STANDARD_RE = re.compile(r'CCTV\D*?(\d{1,2})\s*\+?', re.IGNORECASE)
# M3U文件tvg-name属性提取
...
# ─────────────────────────────────────────────
#  CCTV命名标准化：【终极严格版】仅允许 CCTV5 和 CCTV5+，其余全部纯数字、无+号
# ─────────────────────────────────────────────
def normalize_cctv(name: str) -> str:
    ...
    if not upper_name.startswith('CCTV'):
        return name
    # 使用全局预编译正则，避免重复编译
    match = _CCTV_STANDARD_RE.search(upper_name)
    if not match:
        return name
...
# ─────────────────────────────────────────────
#  频道名清洗（删除冗余的_CCTV_NORM_RE.sub操作）
# ─────────────────────────────────────────────
...
def clean_name(n: str) -> str:
    ...
    for prefix in _OVERSEAS_PREFIX:
        if n.startswith(prefix) and len(n) > len(prefix) + 1:
            ...
            if m:
                ...
                break
    n = _NOISE_RE.sub('', n)
    # 【删除冗余】原_CCTV_NORM_RE.sub操作，效果被后续normalize_cctv完全覆盖
    n = normalize_cctv(n)  # 第一次CCTV标准化
    ...
    n = _SUFFIX_RE.sub('', n)
    n = simplify(n)
    n = normalize_cctv(n)  # 最终标准化，防止所有遗漏情况
