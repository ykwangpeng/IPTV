...
DEBUG_MODE = True              # 改为True，输出详细拉取/解析日志
AUTO_BACKUP = True
ARCHIVE_FAIL = True
SORT_BY_LATENCY_FIRST = True
# 并发&防封禁配置：延长超时，增加重试
MAX_WORKERS = 12                # 降低并发，减少源站风控
MAX_PER_DOMAIN_WORKERS = 2      # 单域名并发再降低，避免被封
TIMEOUT_CN = 30                 # 境内超时延长到30s
TIMEOUT_OVERSEAS = 60           # 境外超时延长到60s
RETRY_COUNT = 3                 # 重试次数增加到3次
REQUEST_JITTER = True
# 源筛选配置：关闭过度清理，避免误判有效源
MAX_LINKS_PER_NAME = 5
FILTER_PRIVATE_IP = True
REMOVE_REDUNDANT_PARAMS = False  # 改为False，不清理URL参数（很多源的参数是必要的）
# 网络源列表（保留原列表，优化拉取逻辑）
WEB_SOURCES = [
    "https://raw.githubusercontent.com/iptv-org/iptv/master/channels/cn.m3u",
    "https://ghproxy.com/https://raw.githubusercontent.com/liuhuanshuo/iptv/main/iptv.m3u",
    "https://live.264788.xyz/channel/fujianjiaoyu?streamid=4b152d0c817d6d729fa6df7d36ee3693&livekey=01Whq5ncXSAY4QR6puuX7Rt5f1wsnZ1eUvizF",
    "https://feer-cdn-bp.xpnb.qzz.io/xnkl.txt",
    "https://php.946985.filegear-sg.me/test.m3u",
    "http://iptv.4666888.xyz/FYTV.m3u",
    "https://ghproxy.com/https://raw.githubusercontent.com/judy-gotv/iptv/main/litv.m3u",
    "https://ghproxy.com/https://raw.githubusercontent.com/develop202/migu_video/refs/heads/main/interface.txt",
...
PROXY = 'http://127.0.0.1:7897'                   #None
# -------------------------- 日志系统初始化（保持不变） --------------------------
def init_logger():
...
def fix_mojibake(text):
    ...
    try:
        utf8_text = text_bytes.decode('utf-8', errors='strict')
    ...
    if detected_encoding and detected_encoding.lower() not in [enc.lower() for enc in support_encodings + ['utf-8']]:
...
def fetch_web_sources(url):
    """核心优化：强化请求头、优化解压、兼容更多格式"""
    try:
        # 1. 强化请求头：模拟真实浏览器，避免被源站识别为爬虫
        ua = random.choice(UA_POOL)
        headers = {
            'User-Agent': ua,
        ...
        proxies = {'http': PROXY, 'https': PROXY} if PROXY else None
        resp = None
        # 2. 增加重试次数+延长超时（修复重复headers参数）
        for retry in range(5):  # 重试5次，应对不稳定网络
            try:
                resp = requests.get(
                    url, 
                    headers=headers,
                    timeout=(15, 90) if "githubusercontent" in url else (15, 60),  # 延长连接/读取超时
                    verify=False, 
                    stream=True, 
                    proxies=proxies, 
                ...
                if DEBUG_MODE:
                    logger.debug(f"⏱️  {url} 第{retry+1}次超时，延迟{sleep_time:.1f}s重试")
                time.sleep(sleep_time)
            except requests.exceptions.RequestException as e:
                # 移除ConnectionResetError单独捕获，避免版本兼容问题
                if retry == 4:
                    raise
                sleep_time = random.uniform(1, 3)
                if DEBUG_MODE:
                    logger.debug(f"❌  {url} 第{retry+1}次请求失败: {str(e)}，延迟{sleep_time:.1f}s重试")
                time.sleep(sleep_time)
        
        if not resp:
            raise Exception("请求重试5次均失败")
        
        # 3. 优化解压：基于魔术数字检测，避免依赖Content-Encoding头
        content = resp.content
        logger.debug(f"📥 拉取 {url} 成功，原始内容长度: {len(content)} 字节")
        # 尝试解压gzip（魔术数字：0x1f8b）
        try:
            if len(content) >= 2 and content[:2] == b'\x1f\x8b':
                import gzip
                content = gzip.decompress(content)
                logger.debug(f"✅ {gzip.__name__} 解压成功，解压后长度: {len(content)} 字节")
        except Exception as e:
            logger.debug(f"⚠️  gzip解压失败: {str(e)}")
        # 尝试解压brotli（魔术数字：0x0b1800）
        try:
            if len(content) >= 3 and content[:3] == b'\x0b\x18\x00':
                import brotli
                content = brotli.decompress(content)
                logger.debug(f"✅ {brotli.__name__} 解压成功，解压后长度: {len(content)} 字节")
        except Exception as e:
            logger.debug(f"⚠️  brotli解压失败: {str(e)}")
        
        # 4. 多编码兼容解码（保持不变）
        text_content = None
        for enc in ['utf-8-sig', 'utf-8']:
            try:
                text_content = content.decode(enc, errors='strict')
                logger.debug(f"✅ 使用编码 {enc} 解码成功")
                break
            except UnicodeDecodeError:
                continue
        if not text_content:
            for enc in ['gb18030', 'gbk', 'big5', 'latin-1', 'ascii']:
                try:
                    text_content = content.decode(enc, errors='strict')
                    logger.debug(f"✅ 使用编码 {enc} 解码成功")
                    break
        ...
        if not text_content:
            ...
            logger.warning(f"⚠️  所有编码严格解码失败，使用兼容模式")
        ...
        logger.info(f"✅ 拉取成功: {url} | 原始{len(parsed)}条 | 去重后{len(unique_parsed)}条源")
    ...
    try:
        ...
        for enc in encode_priority:
            try:
                ...
                logger.info(f"✅ 使用编码 {enc} 成功读取本地文件")
                break
            except UnicodeDecodeError:
                continue
        if text is None:
        ...
        if any(l.strip().startswith('#EXTM3U') for l in lines):
            ...
            # 过滤无效行（非空、包含逗号和链接）
            parsed_lines = [l.strip() for l in lines if l.strip() and "," in l and "://" in l]
        
        # 输出本地文件解析出的源数量
        logger.info(f"📄 本地文件解析出 {len(parsed_lines)} 条有效源")
        return parsed_lines
    except FileNotFoundError:
