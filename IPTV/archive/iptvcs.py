...
WEB_SOURCES = [
    "http://php.946985.filegear-sg.me/test.m3u",
    "https://raw.githubusercontent.com/develop202/migu_video/refs/heads/main/interface.txt",
    "https://gh-proxy.org/https://gist.githubusercontent.com/litywang/dc272a4f2e95ffbd41e7e31d27ef3d76/raw/IPTV.txt",
...
def fix_mojibake(text):
    ...
    original_chinese = sum('\u4e00' <= c <= '\u9fff' for c in original)
    original_ratio = original_chinese / original_len if original_len > 0 else 0

    priority_pairs = [
        ('utf-8', 'gbk'),        # 命中率最高（70%+）
        ('utf-8', 'gb18030'),    # gb18030 更宽松
        ('gbk', 'utf-8'),        # 反向（少数情况）
        ('gb18030', 'utf-8'),
    ]
    ...
    for wrong, correct in priority_pairs:
        try:
            fixed = original.encode(wrong, errors='ignore').decode(correct, errors='ignore').strip()
            chinese = sum('\u4e00' <= c <= '\u9fff' for c in fixed)
            ratio = chinese / len(fixed) if len(fixed) > 0 else 0
            len_diff = abs(len(fixed) - original_len)

            if (ratio > best_ratio + 0.15 and          # 中文比例提升明显
                len_diff <= original_len * 0.35 and     # 长度变化合理
                ratio >= 0.35 and                       # 至少有一定中文
                not any(ord(c) < 32 and c not in '\t\n\r ' for c in fixed)):  # 无大量控制字符
    ...
    extra_pairs = [
        ...
        ('big5', 'utf-8'),
        ('utf-8', 'big5'),
        ('utf-16', 'utf-8'),     # 极少数utf-16误读
    ]

    for wrong, correct in extra_pairs:
        try:
            fixed = original.encode(wrong, 'ignore').decode(correct, 'ignore').strip()
            chinese = sum('\u4e00' <= c <= '\u9fff' for c in fixed)
            ratio = chinese / len(fixed) if len(fixed) > 0 else 0
            len_diff = abs(len(fixed) - original_len)

            if ratio > best_ratio + 0.1 and len_diff <= original_len * 0.4:
                best = fixed
                best_ratio = ratio
                if ratio > 0.7:
                    return best
    ...
    half = ''.join(chr(ord(c) - 0xFEE0) if 0xFF01 <= ord(c) <= 0xFF5E else c for c in best)

    half_chinese = sum('\u4e00' <= c <= '\u9fff' for c in half)
    if half_chinese > best_ratio * len(half) / original_len * 1.2:
        best = half

    cleaned = re.sub(r'[\x00-\x1F\x7F-\x9F]', '', best).strip()

    cleaned_chinese = sum('\u4e00' <= c <= '\u9fff' for c in cleaned)
    if cleaned_chinese > best_ratio * len(cleaned) / original_len * 1.1:
        best = cleaned
