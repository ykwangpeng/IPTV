...
def fix_mojibake(text):
    ...
    for enc in utf8_encodings:
        try:
            ...
            chinese_count = sum('\u4e00' <= c <= '\u9fff' or '\u3400' <= c <= '\u4dbf' for c in utf8_text)
            total_count = len(utf8_text.strip())
            utf8_ratio = chinese_count / total_count if total_count > 0 else 0
            if utf8_ratio >= min_chinese_ratio:
                best_text = utf8_text
                best_text = invalid_chars.sub('', best_text).strip()
                return best_text or "未知频道"
        except UnicodeDecodeError:
            continue
    
    support_encodings = ['gb18030', 'gbk', 'big5', 'big5hkscs']
    ...
    origin_chinese = sum('\u4e00' <= c <= '\u9fff' or '\u3400' <= c <= '\u4dbf' for c in best_text)
    origin_total = len(best_text.strip())
    best_ratio = origin_chinese / origin_total if origin_total > 0 else 0
    
    for enc in support_encodings:
        try:
            temp_text = text_bytes.decode(enc, errors='replace')
            temp_text = temp_text.replace('�', '').strip()
            temp_text = invalid_chars.sub('', temp_text).strip()
            if not temp_text:
                continue
            temp_chinese = sum('\u4e00' <= c <= '\u9fff' or '\u3400' <= c <= '\u4dbf' for c in temp_text)
            temp_total = len(temp_text)
            temp_ratio = temp_chinese / temp_total if temp_total > 0 else 0
    ...
    best_text = invalid_chars.sub('', best_text).strip()
    final_chinese = sum('\u4e00' <= c <= '\u9fff' or '\u3400' <= c <= '\u4dbf' for c in best_text)
    final_total = len(best_text)
    final_ratio = final_chinese / final_total if final_total > 0 else 0
    if final_ratio < min_chinese_ratio and not re.search(r'[A-Za-z0-9]+', best_text):
        return "未知频道"
...
def clean_name(n):
    ...
    if not n or n == "未知频道":
        return "未知频道"
    # 【新增】补充境外高频前缀（覆盖WOWOW/NOW/NHK/博斯），扩大乱码过滤范围
...
def parse_m3u(lines):
    """增强版 M3U 解析：完美支持带 tvg 参数 + 更新日期的格式"""
    parsed = []
    extinf_line = None
    for line in lines:
        line = line.strip()
        ...
        # 匹配任何 #EXTINF 开头的行（不管后面有多少参数）
        if line.startswith('#EXTINF:'):
            extinf_line = line
            continue
        
        # 遇到 URL 时提取名字（取最后一个逗号后的内容）
        if extinf_line and ('http://' in line or 'https://' in line):
            if ',' in extinf_line:
                # 取最后一个逗号后的名字（处理“更新日期: xxx”或“CCTV1综合”）
            ...
            display_name = fix_mojibake(name_part)
...
# 统一解析 - 增强 M3U 检测（完美适配带参数的 #EXTM3U）
        parsed = []
        has_m3u = any('#EXTM3U' in l.upper() for l in lines[:10])  # 只查前10行，更稳
        if has_m3u:
            logger.debug("✅ 检测到 M3U 头，进入 parse_m3u 分支")
