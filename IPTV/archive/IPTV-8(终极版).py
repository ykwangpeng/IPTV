...
class Config:
    ...
    CATEGORY_RULES = {
        ...
        "港澳台頻": [
            ...
            "KOMPASTV", "KMTV", "INEWS", "INDOSIAR", "HUAHEEDAI", "Z频道", "星球",
    ...
    def check(self, line: str, proxy: Optional[str] = None) -> Dict[str, Any]:
        try:
            ...
            if not URLCleaner.is_valid(url):
                return {"status": "失效", "name": name or "未知频道", "url": url, "reason": "URL无效"}
            ...
            if Config.REQUEST_JITTER:
                time.sleep(random.uniform(0.02, 0.1))  # ✅ 减少抖动延迟
...
class IPTVChecker:
    def __init__(self):
        Config.init_compiled_rules()  # ✅ 初始化编译规则
    ...
    def run(self, args: argparse.Namespace) -> None:
        ...
        if Config.WEB_SOURCES and not args.no_web:
            self.logger.info(f"🌐 拉取 {len(Config.WEB_SOURCES)} 个网络源...")
